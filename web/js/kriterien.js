document.addEventListener("DOMContentLoaded", () => {
  ...
  main.innerHTML = `
    <section class="hero">
      <h1>Kaderrichtlinien</h1>
    </section>

    <section class="info-wrap" aria-label="Kaderrichtlinien Übersicht">
      <section class="info-section" aria-labelledby="info-current-title">
        <h2 id="info-current-title">Aktuelle Kaderrichtlinien</h2>
        <div id="info-current" class="info-links">
          <p class="info-status">Lade Kaderrichtlinien</p>
        </div>
      </section>

      <section class="info-section" aria-labelledby="info-archive-title">
        <h2 id="info-archive-title">Frühere Kaderrichtlinien</h2>
  ...
  loadInfoschreiben().catch(() => {
    renderError("Die Kaderrichtlinien konnten nicht geladen werden.");
  });
});
...
async function loadInfoschreiben() {
  ...
  const cfg = {
    ...
    branch: "main",
    dirCandidates: ["kaderkriterien", "web/kaderkriterien"],
    cacheKey: "lsb_kaderkriterien_cache_v1",
  ...
  try {
    ...
    renderError("Die Kaderrichtlinien konnten nicht geladen werden (GitHub API).");
    throw err;
  }
}
...
function renderLists(docs, elCurrent, elArchive) {
  if (!Array.isArray(docs) || docs.length === 0) {
    elCurrent.innerHTML = `<p class="info-status">Keine Kaderrichtlinie gefunden.</p>`;
...
function renderCurrentCard(doc) {
  ...
  const aria = year ? `Kaderrichtlinie ${year} öffnen` : "Kaderrichtlinie öffnen";
  ...
  return `
    <a class="info-brief" href="${href}" target="_blank" rel="noopener noreferrer" aria-label="${aria}">
      <img
        ...
        alt="Kaderrichtlinien öffnen"
        loading="lazy"
        decoding="async"
      />
      <div class="info-brief__overlay" aria-hidden="true">
        <div class="info-brief__t1">Kaderrichtlinien ${escapeHtml(year)}</div>
...
function buildLabel(filename, year) {
  if (Number.isFinite(year)) return `Kaderrichtlinien ${year}`;
